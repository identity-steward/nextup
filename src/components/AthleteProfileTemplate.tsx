...
export default function AthleteProfileTemplate({ athlete, onBack, approvedMedia = [], athleteTags = [] }: AthleteProfileTemplateProps) {
  ...
  const isPremium = athlete.profile_tier === 'premium';

  const visibilityFields = isPremium ? [
    athlete.height ? { label: 'Height', value: athlete.height } : null,
    athlete.jersey_number ? { label: 'Jersey #', value: `#${athlete.jersey_number}` } : null,
    athlete.class_year ? { label: 'Class', value: athlete.class_year } : null,
    athlete.position ? { label: 'Position', value: athlete.position } : null,
    athlete.instagram_handle ? { label: 'Instagram', value: `@${athlete.instagram_handle.replace(/^@/, '')}` } : null,
    athlete.twitter_handle ? { label: 'X / Twitter', value: `@${athlete.twitter_handle.replace(/^@/, '')}` } : null,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {isPremium && visibilityFields.length > 0 && (
        <section className="py-10 bg-white border-t border-gray-100">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <div className="flex items-center gap-3 mb-6">
              <Shield className="w-5 h-5 text-amber-500" />
              <h2 className="text-xl font-bold text-navy">Visibility Profile</h2>
              <span className="inline-flex items-center gap-1 bg-gradient-to-r from-amber-400 to-yellow-300 text-black text-xs font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full">
                <Trophy className="w-3 h-3" />
                Premium
              </span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {visibilityFields.map(({ label, value }) => (
                <div key={label} className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
                  ...
                  <p className="font-bold text-sm text-navy">{value}</p>
                </div>
              ))}
            </div>
          </div>
